...
{
	cout << "initializer queue from list: " << endl;
	list<int> li = {1, 2, 3, 4, 5};
	queue<int, list<int>> ql(li); 	// constuctor copies to new list
	...
	cout << "li has " << li.size() << "entries; ql has " << ql.size() << " entries." << endl;
	cout << "content of li after ql is emptied: " << endl;
	for(auto it = li.begin();it != li.end();it++){ 		// auto is C++11 only
		cout << *it << " ";
	}
	// or
/*	for(list<int>::iterator it = li.begin();it != li.end();it++){ 		// auto is C++11 only
		cout << *it << " ";
	}*/
	cout << endl;

	queue<string> qd; 	// default queue uses deque object
	cout << "push string onto qd" << endl;
	qd.push("one");
	qd.push("two");
	qd.push("three");
	qd.push("four");
